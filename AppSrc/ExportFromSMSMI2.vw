// L:\VDF14.1.Dev\TITAN_MIGRATE\AppSrc\ExportFromSMSMI2.vw
// ExportFromSMSMI2
//
...
ACTIVATE_VIEW Activate_ExportFromSMSMI2 FOR ExportFromSMSMI2

Object ExportFromSMSMI2 is a dbView
    Set Location to 4 9
    Set Size to 122 411
    Set Label to "„¥˜šàšã œ¢˜«é¤, “§œçŸ¬¤à¤ ¢¨à£é¤, ë¨šà¤, ‹®˜¤ ¡é¤, „¨š¦¢á™à¤"
    ...
    Object Button5 is a Button
        ...
        Procedure OnClick
            ...
            While (Found)
                Add 1 to iCnt
                Move ("„¢âš®¦¤«˜  ¦  œšš¨˜­âª... "+String(iCnt)+"/"+String(iFS)) to sMess
                Set Label to sMess
                If (MICN.EXPORTFLAG=1) Begin
                    Clear MICLIENT
                    Move MICN.CLIENT_ID to MIClient.CLIENT_ID
                    Find Eq MICLIENT by Index.6
                    If ((Found) and (MIClient.EXPORTFLAG<>1)) Begin
                        Reread MICLIENT
                        Move 1 to MIClient.EXPORTFLAG
                        SaveRecord MICLIENT
                        Unlock
                    End
                End
            ...
            String sFileName2
            Move "C:\TitanMig\MIMhxan.csv" to sFileName2
            Direct_Output channel 7 sFilename2
        
            String sFileName3
            Move "C:\TitanMig\MIConstr.csv" to sFileName3
            Direct_Output channel 8 sFilename3
        
            String sFileName4
            Move "C:\TitanMig\MISalesm.csv" to sFileName4
            ...
            Clear MIClient
            Find GE MIClient.Recnum
            While (Found)
                Add 1 to iCnt
                Move ("œ¢á«œª... "+String(iCnt)+"/"+String(iFS)) to sMess
                Set Label to sMess  //(Trim(MIClient.Client_ID)+"  "+Trim(MIClient.Client_Name))
                If (MIClient.EXPORTFLAG=1) Send OutputClient
                Find GT MIClient.Recnum
            Loop
