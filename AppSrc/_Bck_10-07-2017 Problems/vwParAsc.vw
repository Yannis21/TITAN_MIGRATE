...
//     Wapokat_Wapokat_Code is a dbFormSub
//     Wapokat_Wapokat_Descr is a dbFormSub
...
//AB/     AppBuild VIEW
    ...
    Object vwParAsc is a dbViewSubs
        ...
        Object Wapokat_Wapokat_Code is a dbFormSub
            ...
            Set Label to "âò´ûö¶®Âò"
            Set Size to 13 95
            Set Location to 92 83
            Set Label_Col_Offset to 2
            Set Label_Justification_Mode to jMode_Right
            Set Form_TypeFace Item 0 to "MS Sans Serif"
        ...
        Object Wapokat_Wapokat_Descr is a dbFormSub
            Entry_Item Wapokat.Wapokat_descr
            Set Server to (Wapokat_DD(self))
            Set Size to 13 229
            Set Location to 92 186
            Set Color to clBtnFace
            Set Label_Col_Offset to 2
            Set Label_Justification_Mode to jMode_Right
            Set Enabled_State to FALSE
        ...
        Object oButton1 is a ButtonSub
            ...
            Procedure OnClick
                Date ddApo ddEos
                String sClient sErgo sPtyppar sFileName sLabel sD sM sY sWapokat
                String sQty sQtyDec sStr sAFM lsStr sssOra
                Integer iCount iCNCode iH iM
                ...
                Get Field_Current_Value Of Wapokat_DD Field Wapokat.Wapokat_Code to sWapokat
                Move (Trim(sWapokat)) to sWapokat
            
                If (sClient="") Begin
                    Send Stop_Box "Ä§Áßò®°´¶™ ßú¢·´û™" "Error 1001"
                    Procedure_Return
                End
                If (sPtyppar="") Begin
                    Send Stop_Box "Ä§Áßò®°´¶ ßò®ò©´ò´†°Ê" "Error 1006"
                    Procedure_Return
                End
                If (ddApo="") Begin
                    Send Stop_Box "Ü£/§Âò Äß¶ úÂ§ò† °ú§„" "Error 1002"
                    Procedure_Return
                End
                If (ddEos="") Begin
                    Send Stop_Box "Ü£/§Âò Ñ‡™ úÂ§ò† °ú§„" "Error 1003"
                    Procedure_Return
                End
                If (ddApo>ddEos) Begin
                    Send Stop_Box "Ü£/§Âò Äß¶ £úöò¢Á´ú®û òß¶ Ü£/§Âò Ñ‡™" "Error 1004"
                    Procedure_Return
                End
                If (sFileName="") Begin
                    Send Stop_Box "Ó§¶£ò ò®ÆúÂ¶¨ °ú§Ê" "Error 1005"
                    Procedure_Return
                End
                ...
                While ((Found) and (Particle.Partl_Hmnia<=ddEos))
                    If ((sClient=Particle.Client_ID)                        and ;
                        ...
                        // Yannis 28/03/2017
                        Move (Trim(Particle.Ora_Apostolhs)) to sssOra
                        If (Length(sssOra)=4) Insert ":" in sssOra at 3
                        If (Trim(sssOra)="") Move "     " to sssOra
                        // Yannis 28/03/2017
                        ...
                        While ((Found=1) and (Particle.Partl_Recr=Pmoves.Partl_Recr))
                            ...
                            // Prosthiki 24/09/2013 Filtraroume tora mono
                            // ta etoima proionta kai exagoume pinakida oxhmatos kai antlias
                            If ((sWapokat="") Or (Pmoves.Wapokat_Code=sWapokat)) Begin
                                ...
                                String sCar sPump
                                Clear Cars
                                Move Particle.Oxhma_Code to Cars.Car_ID
                                Find Eq Cars By Index.2
                                Move (Trim(Cars.Car_License)) to sCar
            
                                Clear Pumps
                                Move Particle.Antlia_Code to Pumps.Pump_ID
                                Find Eq Pumps By Index.3
                                Move (Trim(Pumps.Pump_License)) to sPump
            
                                // P.S. 01/10/2013 Theloume akoma 2 kena gia na arxizei apo thn sthlh 64
                                Write   Channel 9 "  "
                                // P.S. 01/10/2013 Theloume akoma 2 kena gia na arxizei apo thn sthlh 64
                                Write   Channel 9 (Pad(sCar , 9))
                                Write   Channel 9 (Pad(sPump, 9))
            
            
                                // Yannis 28/03/2017
                                Write   Channel 9 (Pad(sssOra,5))
                                // Yannis 28/03/2017
        ...
        Procedure Activate_View Returns Integer
            ...
            Clear Wapokat
            Move Sysfile.Wapokat_Etoima to Wapokat.Wapokat_Code
            Find Eq Wapokat By Index.1
            Send Find Of Wapokat_DD Eq 1
