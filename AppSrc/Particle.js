...
//AB/     AppBuild VIEW
    ...
    Object Particle is a dbViewSub
        ...
        Object Particle_Partl_Hmnia is a dbFormSub
            ...
            Procedure Prompt
                String sPtyppar sClient
                Date    dHmnia
                Set psModule    Of (sl014(self)) to (psModule(Self))
                Set Modal_State Of (sl014(self)) to True
            
                Get Value               Of Particle_Partl_Hmnia Item 0           to dHmnia
                // Den pairnoume to field_current_value giati apo thn field defaults
                // prolabainei kai pernei timh hmeras akoma kai an einai keno to pedio
                ...
                Set psClient    Of sl014 to (Trim(sClient))
                Set psPtyppar   Of sl014 to (Trim(sPtyppar))
                Set pdHmnia     Of sl014 to dHmnia
            ...
            Procedure Refresh Integer iMode
                ...
                // 27/03/2002 gia na mhn anoigokleinoun palia parastatika
                // tora pou ftiaxnoume ta nea theorimena
            
            // jsjsjsjs 12/12/2002
            //            Move '26/03/2002' to dHmnia2
            //            If (particle.partl_hmnia<dHmnia2) Begin
            //                Set Enabled_State   Of BtnEktyposh          to False
            //                Set Enabled_State   Of BtnKleisimo          to False
            //            End
            ...
            //    // jsjsjsjs 12/12/2002
            //    If (Particle.Partl_Ekleise='1') Send Kleise_Ta_Pedia
            //    Else                            Send Anoixe_Ta_Pedia
            
                // jsjsjsjs 21/12/2002
                // ta afinoume telika kleista giati allios xreiazetai
                // kodikas enhmeroseis pmoves an allaxei hmerhmhnia h
                // alla pedia pou katebainoun kato sto pmoves
            ...
            //            End
            ///////////////////////////////////////
            ///////////// 29/01/2003 //////////////  // Refresh Hmnias Parast
            ///////////////////////////////////////
        ...
        Object Clients_Client_ID is a dbFormSub
            ...
            //            Procedure Check_OverHead
            //                If (Ctypkin.Check_OverHead='1') Begin
            //                    If ((Clients.Plafon>0) And (Clients.OverHead<1) And (Sysfile.Over_OverHead<>'1')) Begin
            //                        Send Stop_Box "é ßú¢·´û™ õú§ ‚Æú† õ†òü‚©†£û ßÂ©´‡©û.\ní¶ ß®Êö®ò££ò õú§ üò òß¶üû°úÁ©ú† ´¶ ßò®ò©´ò´†°Ê \nò§ ßò®ò£úÂ§ú† ¶ ©¨ö°ú°®†£‚§¶™ ßú¢·´û™" "4050  ** è®¶©¶Æ„ **"
            //                    End
            //                    Else If ((PayResp.Plafon>0) And (PayResp.OverHead<1) And (Sysfile.Over_OverHead<>'1')) Begin // And (Particle.Recnum=0)) Begin
            //                        Send Stop_Box "é ìßúÁü¨§¶™ è¢û®‡£È§ õú§ ‚Æú† õ†òü‚©†£û ßÂ©´‡©û.\ní¶ ß®Êö®ò££ò õú§ üò òß¶üû°úÁ©ú† ´¶ ßò®ò©´ò´†°Ê \nò§ ßò®ò£úÂ§ú† ¶ ©¨ö°ú°®†£‚§¶™ ß®¶£ûüú¨´„™" "4060  ** è®¶©¶Æ„ **"
            //                    End
            //                End
            //            End_Procedure
        ...
        Object Particle_Cn_Code is a dbFormSub
            ...
            Procedure Prompt
                string sid
                get field_current_value of clients_dd field clients.client_id to sid
                Set FragmaClient_ID Of (sl041(self)) to sID
                Forward Send Prompt
                Send Show_CN
            End_Procedure
        ...
        Object btnKleisimo is a ButtonSub
            ...
            Procedure OnClick
                ...
                Integer isClosed iErr iRecnum iStatuspanel  iValidateStructure
                Integer iLines iYear iYes iCheck iError_Object_ID iPmsRecnum
                Date    dHmnia dHmnia2
            
                Get Validate_save_structure_mode Of Pmoves_DD to iValidateStructure
                Set Validate_save_structure_mode Of Pmoves_DD to DD_validate_structure_never
                ...
                If (sRecr<>'') Begin
                    ...
                    Get Field_Current_Value Of Pmoves_DD Field Pmoves.Recnum  to iPmsRecnum
            ...
            //        Send Request_Superfind to Particle_DD eq Particle.File_Number Field Particle.Recnum
                    Send Find_By_Recnum to Particle_DD Particle.File_Number iRecnum
            
            
                    If ((Not(iErr)) And (Not(Err)) And (piSfalma(Self)=0)) Begin
                        Get Field_Current_Value Of Particle_DD  Field Particle.Partl_Ekleise    to isClosed
                    ...
                    Else Begin
                        Indicate Err False
                        Move 0 to iErr
                        Send Clear_All to Particle_DD
                        Send Find_By_Recnum to Particle_DD Particle.File_Number iRecnum
                    End
            
                    Set Validate_save_structure_mode Of Pmoves_DD to iValidateStructure
            ...
            //*    must be removed
            //        Send Clear to Tparticl_DD
            //        Send rebuild_Constraints to Tparticl_DD
            //        Send Find to Tparticl_DD First_Record 1
            ...
            //    Send Find to Psxetika_DD First_Record 0
            //    Send Rebuild_Constraints            to PmsSxet_DD
            //    Send Find to PmsSxet_DD First_Record 0
            //**
            ...
            Procedure OnClick
                ...
                If Status Pmoves Begin
                    ...
                    If ((sRecr<>'') And (sID<>'')) Begin
                        ...
                        Get Field_Current_Value Of Particle_DD  Field Particle.Timol_Ypol  to isTimol
                        Get Field_Current_Value Of Particle_DD  Field Particle.Partl_Hmnia to dHmnia
            
                        If (Not(isTimol)) Begin
